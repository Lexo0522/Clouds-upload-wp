document.addEventListener('DOMContentLoaded', function() {
    ...
    function renderStats(stats) {
        ...
        // 更新流量统计
        updateTrafficStats(normalizeTrafficStats(stats.traffic));
    ...
    // 更新流量统计
    function normalizeTrafficStats(traffic) {
        if (!traffic) return traffic;

        var normalized = {
            today: traffic.today ? {
                bytes: parseInt(traffic.today.bytes, 10) || 0,
                bytes_formatted: traffic.today.bytes_formatted || sizeFormat(traffic.today.bytes),
                requests: parseInt(traffic.today.requests, 10) || 0
            } : null,
            week: traffic.week ? {
                bytes: parseInt(traffic.week.bytes, 10) || 0,
                bytes_formatted: traffic.week.bytes_formatted || sizeFormat(traffic.week.bytes),
                requests: parseInt(traffic.week.requests, 10) || 0
            } : null,
            month: traffic.month ? {
                bytes: parseInt(traffic.month.bytes, 10) || 0,
                bytes_formatted: traffic.month.bytes_formatted || sizeFormat(traffic.month.bytes),
                requests: parseInt(traffic.month.requests, 10) || 0
            } : null,
            daily: {},
            by_type: {}
        };
        ...
        if (traffic.daily) {
            Object.keys(traffic.daily).forEach(function(date) {
                var item = traffic.daily[date] || {};
                normalized.daily[date] = {
                    bytes: parseInt(item.bytes, 10) || 0,
                    bytes_formatted: item.bytes_formatted || sizeFormat(item.bytes),
                    requests: parseInt(item.requests, 10) || 0
                };
            });
        }

        if (traffic.by_type) {
            Object.keys(traffic.by_type).forEach(function(type) {
                var item = traffic.by_type[type] || {};
                normalized.by_type[type] = {
                    bytes: parseInt(item.bytes, 10) || 0,
                    bytes_formatted: item.bytes_formatted || sizeFormat(item.bytes),
                    requests: parseInt(item.requests, 10) || 0
                };
    ...
    function renderCharts(stats) {
        ...
        // 每日流量趋势图
        renderTrafficChart(normalizeTrafficStats(stats.traffic));
    }
