...
class WPMCS_Attachment_Manager {
	...
	/**
	 * Build the SQL condition used to detect cloud-backed attachments.
	 *
	 * This checks both the current postmeta record and the fallback copy that
	 * is stored inside `_wp_attachment_metadata` so uploads remain recognized
	 * after plugin reinstall/uninstall cycles.
	 *
	 * @param string $post_id_column SQL expression for the attachment ID column.
	 * @return string
	 */
	public static function get_cloud_detection_sql( $post_id_column = '' ) {
		global $wpdb;

		$post_id_column = '' !== $post_id_column ? $post_id_column : "{$wpdb->posts}.ID";

		return "( EXISTS (
			SELECT 1
			FROM {$wpdb->postmeta} wpmcs_meta
			WHERE wpmcs_meta.post_id = {$post_id_column}
			AND wpmcs_meta.meta_key IN ('_wpmcs_cloud_meta', '_wpmcs_cloud_url')
			AND wpmcs_meta.meta_value != ''
		) OR EXISTS (
			SELECT 1
			FROM {$wpdb->postmeta} wpmcs_meta
			WHERE wpmcs_meta.post_id = {$post_id_column}
			AND wpmcs_meta.meta_key = '_wp_attachment_metadata'
			AND wpmcs_meta.meta_value LIKE '%wpmcs_cloud%'
		) )";
	}
	...
	public static function get_all_cloud_attachments() {
		...
		$attachment_ids = $wpdb->get_col(
			...
			AND " . self::get_cloud_detection_sql()
		);
