 ...
 * AWS S3 适配器
 */
class WPMCS_AWS_S3_Adapter extends WPMCS_Cloud_Adapter {
	...
	public function upload( $local_path, $cloud_key, $mime_type = '' ) {
		if ( ! $this->is_configured() ) {
			return new WP_Error( 'wpmcs_aws_s3_not_configured', 'AWS S3 配置不完整。' );
		}
		...
		// 创建 S3 存储实例
		$storage = new AWS_S3_Storage( array(
			'access_key' => $this->settings['access_key'],
			'secret_key' => $this->settings['secret_key'],
			'bucket'     => $this->settings['bucket'],
			'region'     => isset( $this->settings['region'] ) ? $this->settings['region'] : 'us-east-1',
		...
		try {
			...
			return new WP_Error( 'wpmcs_aws_s3_upload_failed', $e->getMessage() );
		}
	}
}