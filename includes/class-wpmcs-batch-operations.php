...
class WPMCS_Batch_Operations {
	...
	public function ajax_batch_migrate() {
		...
		// 获取未上传的附件
		$uploaded_ids = class_exists( 'WPMCS_Attachment_Manager' ) ? WPMCS_Attachment_Manager::get_all_cloud_attachments() : array();
		$args = array(
		...
		if ( ! empty( $uploaded_ids ) ) {
			$args['post__not_in'] = $uploaded_ids;
		}
	...
	public function ajax_get_batch_status() {
		...
		// 已上传数量
		$uploaded_count = class_exists( 'WPMCS_Attachment_Manager' )
