...
class WPMCS_REST_API {
	...
	public function get_files( $request ) {
		...
		if ( $status === 'uploaded' ) {
			if ( class_exists( 'WPMCS_Attachment_Manager' ) ) {
				$where .= ' AND ' . WPMCS_Attachment_Manager::get_cloud_detection_sql( "{$wpdb->posts}.ID" );
			} else {
			...
			if ( class_exists( 'WPMCS_Attachment_Manager' ) ) {
				$where .= ' AND NOT ' . WPMCS_Attachment_Manager::get_cloud_detection_sql( "{$wpdb->posts}.ID" );
			} else {
		...
		foreach ( $files as $file ) {
			$cloud_meta = class_exists( 'WPMCS_Attachment_Manager' )
				? WPMCS_Attachment_Manager::get_cloud_meta( $file->ID )
				: get_post_meta( $file->ID, '_wpmcs_cloud_meta', true );
	...
	public function get_file( $request ) {
		...
		$cloud_meta = class_exists( 'WPMCS_Attachment_Manager' )
			? WPMCS_Attachment_Manager::get_cloud_meta( $id )
			: get_post_meta( $id, '_wpmcs_cloud_meta', true );
