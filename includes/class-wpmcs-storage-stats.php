...
class WPMCS_Storage_Stats {
	...
	public function get_storage_usage() {
		global $wpdb;
		return $this->calculate_storage_usage();
		...
		if ( $cloud_files_size > 0 ) {
			...
			$cloud_attachment_ids = class_exists( 'WPMCS_Attachment_Manager' )
				? WPMCS_Attachment_Manager::get_all_cloud_attachments()
				: $wpdb->get_col(
	...
	private function calculate_storage_usage() {
		...
		foreach ( $cloud_attachment_ids as $attachment_id ) {
			...
			if ( $file_size <= 0 ) {
				...
				if ( is_array( $metadata ) && ! empty( $metadata['wpmcs_cloud']['file_size'] ) ) {
					$file_size = (int) $metadata['wpmcs_cloud']['file_size'];
				}
			}
	...
	public function get_file_stats() {
		...
		// 已上传到云端的附件数
		$cloud_attachments = class_exists( 'WPMCS_Attachment_Manager' )
			? count( WPMCS_Attachment_Manager::get_all_cloud_attachments() )
			: (int) $wpdb->get_var(
	...
	public function get_traffic_stats() {
		global $wpdb;
		return $this->build_traffic_stats();
		
		// 检查流量表是否存在
		if ( ! $this->traffic_table_exists() ) {
	 ...
	 * 获取服务商统计
	 */
	private function build_traffic_stats() {
		global $wpdb;
		...
		$now = current_time( 'timestamp' );
		$today_start_ts = strtotime( wp_date( 'Y-m-d 00:00:00', $now ) );
		$today_end_ts = strtotime( '+1 day', $today_start_ts );
		$week_start_ts = strtotime( wp_date( 'Y-m-d 00:00:00', strtotime( 'monday this week', $now ) ) );
		$month_start_ts = strtotime( wp_date( 'Y-m-01 00:00:00', $now ) );
		$chart_start_ts = strtotime( '-29 days', $today_start_ts );
		$query_start_ts = min( $chart_start_ts, $month_start_ts );

		$range_start = wp_date( 'Y-m-d H:i:s', $query_start_ts );
		$range_end = wp_date( 'Y-m-d H:i:s', $today_end_ts );

		$rows = $wpdb->get_results( $wpdb->prepare(
			"SELECT DATE(created_at) as date, file_type, COALESCE(SUM(bytes), 0) as bytes, COUNT(*) as requests
			FROM {$this->traffic_table}
		...
		if ( $rows ) {
			foreach ( $rows as $row ) {
				...
				$daily_stats[ $date ]['bytes'] += $bytes;
				$daily_stats[ $date ]['requests'] += $requests;
				$daily_stats[ $date ]['bytes_formatted'] = size_format( $daily_stats[ $date ]['bytes'] );

				if ( ! isset( $type_stats[ $file_type ] ) ) {
					$type_stats[ $file_type ] = array(
						'bytes' => 0,
						'bytes_formatted' => '0 B',
						'requests' => 0,
					);
				...
				$type_stats[ $file_type ]['bytes'] += $bytes;
				$type_stats[ $file_type ]['requests'] += $requests;
				$type_stats[ $file_type ]['bytes_formatted'] = size_format( $type_stats[ $file_type ]['bytes'] );
			}
		}

		$today_traffic = $this->get_traffic_range_totals(
			wp_date( 'Y-m-d H:i:s', $today_start_ts ),
			wp_date( 'Y-m-d H:i:s', $today_end_ts )
		);
		if ( 0 === $today_traffic['bytes'] && 0 === $today_traffic['requests'] ) {
			$today_traffic = $this->get_traffic_range_totals_utc( $today_start_ts, $today_end_ts );
		}

		$week_traffic = $this->get_traffic_range_totals(
			wp_date( 'Y-m-d H:i:s', $week_start_ts ),
			wp_date( 'Y-m-d H:i:s', $today_end_ts )
		);
		if ( 0 === $week_traffic['bytes'] && 0 === $week_traffic['requests'] ) {
			$week_traffic = $this->get_traffic_range_totals_utc( $week_start_ts, $today_end_ts );
		}

		$month_traffic = $this->get_traffic_range_totals(
			wp_date( 'Y-m-01 00:00:00', $now ),
			wp_date( 'Y-m-d H:i:s', $today_end_ts )
		);
		if ( 0 === $month_traffic['bytes'] && 0 === $month_traffic['requests'] ) {
			$month_traffic = $this->get_traffic_range_totals_utc( $month_start_ts, $today_end_ts );
		}

		$chart_start = wp_date( 'Y-m-d', $chart_start_ts );
		$daily_stats = array_filter(
			$daily_stats,
			static function( $stats, $date ) use ( $chart_start ) {
				return $date >= $chart_start;
			},
	...
	private function get_traffic_range_totals( $start, $end ) {
		global $wpdb;
	...
	private function get_traffic_range_totals_utc( $start_ts, $end_ts ) {
		return $this->get_traffic_range_totals(
	...
	private function traffic_table_exists() {
		global $wpdb;

		$table = $wpdb->get_var( $wpdb->prepare(
			'SHOW TABLES LIKE %s',
			$wpdb->esc_like( $this->traffic_table )
		) );

		return $table === $this->traffic_table;
	}
