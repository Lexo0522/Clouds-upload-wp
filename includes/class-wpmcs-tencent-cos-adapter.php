<?php

if ( ! defined( 'ABSPATH' ) ) {
	exit;
}

/**
 * 腾讯云 COS 适配器
 */
class WPMCS_Tencent_COS_Adapter extends WPMCS_Cloud_Adapter {
	
	/**
	 * 获取服务商标识
	 */
	public function get_provider_key() {
		return 'tencent_cos';
	}
	
	/**
	 * 检查配置是否完整
	 */
	public function is_configured() {
		$required = array( 'secret_id', 'secret_key', 'bucket', 'region' );
		
		foreach ( $required as $field ) {
			if ( empty( $this->settings[ $field ] ) ) {
				return false;
			}
		}
		
		return true;
	}
	
	/**
	 * 上传文件
	 */
	public function upload( $local_path, $cloud_key, $mime_type = '' ) {
		if ( ! $this->is_configured() ) {
			return new WP_Error( 'wpmcs_tencent_cos_not_configured', '腾讯云 COS 配置不完整。' );
		}
		
		if ( ! file_exists( $local_path ) || ! is_readable( $local_path ) ) {
			return new WP_Error( 'wpmcs_file_missing', '待上传文件不存在或不可读。' );
		}
		
		if ( ! function_exists( 'curl_init' ) ) {
			return new WP_Error( 'wpmcs_curl_missing', '当前 PHP 环境未启用 cURL 扩展。' );
		}
		
		// 创建 COS 存储实例
		$storage = new Tencent_COS_Storage( array(
			'secret_id'  => $this->settings['secret_id'],
			'secret_key' => $this->settings['secret_key'],
			'bucket'     => $this->settings['bucket'],
			'region'     => isset( $this->settings['region'] ) ? $this->settings['region'] : 'ap-beijing',
			'domain'     => isset( $this->settings['domain'] ) ? $this->settings['domain'] : '',
		) );
		
		try {
			$url = $storage->upload( $local_path, $cloud_key );
			
			return array(
				'provider' => $this->get_provider_key(),
				'key'      => $cloud_key,
				'url'      => $url,
			);
			
		} catch ( Exception $e ) {
			return new WP_Error( 'wpmcs_tencent_cos_upload_failed', $e->getMessage() );
		}
	}
}